...
class ScaledDotProductAttention(torch.nn.Module):
    ...
    def forward(self, query, key, value, mask=None) -> torch.Tensor:
        ...
        weight = torch.matmul(query, key.transpose(-2, -1)) / torch.sqrt(torch.tensor(d_k, dtype=torch.float32))
        if mask is not None:
        ...
        assert value.size(-2) == weight.size(-1), "The num of sequence in value must be equal to the num of sequence in query and key"
